...
{
    loff_t pos = 0;
    return kernel_read(fp, buf, fp->f_inode->i_size, &pos);
}
...
static void handle_directory(struct http_request *request)
{
    struct file *fp;
    ...
    if (S_ISDIR(fp->f_inode->i_mode)) {
        ...
        //printk(KERN_INFO "File size: %lu\n", fp->f_inode->i_size);
        char *read_data = kmalloc(fp->f_inode->i_size + 1, GFP_KERNEL);
